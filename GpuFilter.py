...
def run_filter(input_image):
    ...
    axis[0].plot(seq,np.array(med_arr_cpu),'b',label ="cpu")
    axis[0].plot(seq,np.array(med_arr_gpu),'r',label ="gpu")
    axis[0].set_title("Không có io cho bộ lọc trung vị")
    axis[0].legend(fancybox=True)

    axis[1].plot(seq,np.array(med_arr_cpu_io),'b',label ="cpu")
    axis[1].plot(seq,np.array(med_arr_gpu_io),'r',label ="gpu")
    axis[1].set_title("Có io cho bộ lọc trung vị")
    axis[1].legend(fancybox=True)

    
    axis[1].legend(fancybox=True)
    plt.show()
    print(np.average(med_arr_cpu))
    print(np.average(med_arr_gpu))
    print(np.average(med_arr_cpu_io))
    print(np.average(med_arr_gpu_io))
